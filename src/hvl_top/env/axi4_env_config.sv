...
//-------------------------------------------------------
  ...
  extern function void allocate_configs();
...
//--------------------------------------------------------------------------------------------
// Function: allocate_configs
// Purpose : Allocate and create master/slave agent config arrays
//--------------------------------------------------------------------------------------------
function void axi4_env_config::allocate_configs();

  // Allocate arrays
  axi4_master_agent_cfg_h = new[no_of_masters];
  axi4_slave_agent_cfg_h  = new[no_of_slaves];

  // Create master agent configs
  foreach (axi4_master_agent_cfg_h[i]) begin
    axi4_master_agent_cfg_h[i] =
      axi4_master_agent_config::type_id::create(
        $sformatf("master_agent_cfg[%0d]", i));
    axi4_master_agent_cfg_h[i].master_id = i;
  end

  // Create slave agent configs
  foreach (axi4_slave_agent_cfg_h[i]) begin
    axi4_slave_agent_cfg_h[i] =
      axi4_slave_agent_config::type_id::create(
        $sformatf("slave_agent_cfg[%0d]", i));
    axi4_slave_agent_cfg_h[i].slave_id = i;
  end

endfunction : allocate_configs
