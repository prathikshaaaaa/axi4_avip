   ...
   always_ff @(posedge aclk or negedge aresetn) begin 
    if (!aresetn) begin
        for (int s = 0; s < NO_OF_SLAVES; s++) begin
            ...
            wr_slave_busy[s]     = 1'b0;  // ← add
        end
    end else begin
        for (int s = 0; s < NO_OF_SLAVES; s++) begin
            wr_just_released[s] = 1'b0;
            // Clear busy when wlast accepted — all inputs, no delta issue
            if (wr_slave_busy[s]) begin
                for (int m = 0; m < NO_OF_MASTERS; m++) begin
                    if (wr_w_slave[m] == s   &&
                        m_wvalid[m]          &&
                        m_wready[m]          &&
            ...
            // Gate select_master until slave is free
            if (wr_active_master[s] == -1 && !wr_just_released[s] && !wr_slave_busy[s]) begin
                ...
                wr_slave_busy[s]    = 1'b1;  // ← set busy on handshake
            end
        end
    end
end
